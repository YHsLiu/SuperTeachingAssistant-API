...
public class RecordRepository implements RecordDao {
    ...
    public List<Map<String, Object>> RollCallRecordForSemester(int cid) {
        String sql = "select a.姓名,a.學號,b.日期 from 學生資料 a,(select sid,日期 from 點名紀錄 where cid=?) b where a.sid=b.sid order by b.日期,a.學號;";
    ...
    public List<Map<String, Object>> RollCallRecordForToday(int cid, String dd) {
        String sql = "select 姓名,學號 from 學生資料 where sid in (select sid from 點名紀錄 where cid=? and 日期=?;";
